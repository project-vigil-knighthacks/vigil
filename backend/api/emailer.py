...
def mailGun_Config() -> dict:
    ...
    if not mg_api_key or not mg_domain or not mg_sender:
        raise HTTPException(status_code=500, detail="MAILGUN config missing")
...
def send_alert_email(events: list[dict]):
    ...
    return {"ok": True, "alerts_checked": len(events), "recipients": len(users)}

def send_event_alert(event: dict, email: str):
    cfg = mailGun_Config()
    ...
    resp = requests.post(
        ...
        data={
            "from": cfg["sender"],
            "to": email,
            "subject": "Vigil Alert " + event.get("severity", "unknown"),
            "text": "Vigil SIEM Alert\n"
                    "-----------------\n"
